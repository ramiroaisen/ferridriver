...
impl BrowserInstance {
  fn context(&self, name: &str) -> crate::Result<&BrowserContext> {
    self
      .contexts
      .get(name)
      .ok_or_else(|| format!("Context '{name}' not found in this instance.").into())
  }
  ...
  fn context_mut_checked(&mut self, name: &str) -> crate::Result<&mut BrowserContext> {
    self
      .contexts
      .get_mut(name)
      .ok_or_else(|| format!("Context '{name}' not found.").into())
  }
...
impl BrowserState {
  ...
  pub async fn ensure_instance(&mut self, instance_name: &str) -> crate::Result<()> {
  ...
  pub async fn ensure_browser(&mut self) -> crate::Result<()> {
  ...
  pub async fn connect_to_url(&mut self, instance_name: &str, url: &str) -> crate::Result<usize> {
    use crate::backend::cdp::{CdpBrowser, ws::WsTransport};
  ...
  async fn connect_with_resolved_mode(&mut self, instance_name: &str, mode: ConnectMode) -> crate::Result<usize> {
    match mode {
      ...
      ConnectMode::Launch => {
        Err(format!("Instance resolver returned Launch mode for '{instance_name}', expected ConnectUrl").into())
      },
    }
  }
  ...
  fn instance(&self, name: &str) -> crate::Result<&BrowserInstance> {
    self
      .instances
      .get(name)
      .ok_or_else(|| format!("Browser instance '{name}' not found. It will be created on first use.").into())
  }
  ...
  fn instance_mut(&mut self, name: &str) -> crate::Result<&mut BrowserInstance> {
    self
      .instances
      .get_mut(name)
      .ok_or_else(|| format!("Browser instance '{name}' not found.").into())
  }
  ...
  pub async fn open_page(&mut self, context: &str, url: &str) -> crate::Result<AnyPage> {
  ...
  pub fn page_open_plan(&self, key: &SessionKey) -> crate::Result<PageOpenPlan> {
  ...
  pub async fn open_page_keyed(&mut self, key: &SessionKey, url: &str) -> crate::Result<AnyPage> {
  ...
  pub fn active_page(&self, context: &str) -> crate::Result<&AnyPage> {
    let key = SessionKey::parse(context);
    let inst = self.instance(&key.instance)?;
    let ctx = inst.context(&key.context)?;
    ctx
      .active_page()
      .ok_or_else(|| format!("No pages in context '{context}'").into())
  }

  /// # Errors
  ///
  /// Returns an error if the instance or context does not exist.
  pub fn context(&self, context: &str) -> crate::Result<&BrowserContext> {
    let key = SessionKey::parse(context);
    let inst = self.instance(&key.instance)?;
  ...
  ///
  /// Returns an error if the instance or context does not exist.
  pub fn context_mut_checked(&mut self, context: &str) -> crate::Result<&mut BrowserContext> {
    let key = SessionKey::parse(context);
    let inst = self.instance_mut(&key.instance)?;
  ...
  pub fn select_page(&mut self, context: &str, page_idx: usize) -> crate::Result<()> {
    let key = SessionKey::parse(context);
    let inst = self.instance_mut(&key.instance)?;
  ...
  pub fn close_page(&mut self, context: &str, page_idx: usize) -> crate::Result<()> {
    let key = SessionKey::parse(context);
    let inst = self.instance_mut(&key.instance)?;
    let ctx = inst.context_mut_checked(&key.context)?;
    if ctx.pages.len() <= 1 {
    ...
    if page_idx >= ctx.pages.len() {
      return Err(format!("Page index {page_idx} out of range").into());
    }
  ...
  ///
  /// Returns an error if the instance or context does not exist.
  pub async fn network_requests(&self, context: &str, limit: usize) -> crate::Result<Vec<Request>> {
    let key = SessionKey::parse(context);
    let inst = self.instance(&key.instance)?;
    let ctx = inst.context(&key.context)?;
  ...
  pub async fn refresh_pages(&mut self, context: &str) -> crate::Result<usize> {
    let key = SessionKey::parse(context);
    let inst = self.instance_mut(&key.instance)?;
  ...
  ///
  /// Returns an error if the instance or context does not exist.
  pub async fn dialog_messages(&self, context: &str, limit: usize) -> crate::Result<Vec<DialogEvent>> {
    let key = SessionKey::parse(context);
    let inst = self.instance(&key.instance)?;
    let ctx = inst.context(&key.context)?;
...
async fn discover_ws_from_http(http_url: &str) -> crate::Result<String> {
  ...
  let n = buf_reader
    .read(&mut body)
    .await
    .map_err(|e| crate::error::FerriError::from(e))?;
  let body_str = String::from_utf8_lossy(&body[..n]);

  let json: serde_json::Value = serde_json::from_str(&body_str).map_err(crate::error::FerriError::from)?;

  json
    .get("webSocketDebuggerUrl")
    .and_then(|v| v.as_str())
    .map(std::string::ToString::to_string)
    .ok_or_else(|| crate::error::FerriError::backend("No webSocketDebuggerUrl in /json/version"))
}

/// Discover a running Chrome instance by reading its `DevToolsActivePort` file.
fn discover_chrome_ws(channel: &str, explicit_user_data_dir: Option<&str>) -> crate::Result<String> {
  ...
  if lines.len() < 2 {
    return Err(format!("Invalid DevToolsActivePort content: {content:?}").into());
  }

  let port: u16 = lines[0]
    .parse()
    .map_err(|_| crate::error::FerriError::backend(format!("Invalid port '{}' in DevToolsActivePort", lines[0])))?;
  let path = lines[1];

  Ok(format!("ws://127.0.0.1:{port}{path}"))
}

fn chrome_default_user_data_dir(channel: &str) -> crate::Result<std::path::PathBuf> {
  ...
  let suffix = match channel {
    ...
    "dev" => " Dev",
    "canary" => " Canary",
    other => return Err(format!("Unknown Chrome channel: {other}").into()),
  };
  ...
  if !path.exists() {
    let chromium_path = match os {
      ...
      _ => return Err(format!("Chrome user data dir not found: {}", path.display()).into()),
    };
...
pub fn detect_firefox() -> crate::Result<String> {
