...
impl IpcClient {
  ...
  #[cfg(target_os = "macos")]
  fn resolve_host_binary() -> crate::Result<std::path::PathBuf> {
    ...
    if let Ok(path) = std::env::var("FERRIDRIVER_WEBKIT_HOST") {
      ...
      return Err(format!("FERRIDRIVER_WEBKIT_HOST={path} does not exist").into());
    }
  ...
  /// the probe timeout.
  pub async fn spawn(headless: bool) -> crate::Result<(Self, std::process::Child)> {
  ...
  pub async fn send(&self, op: Op, payload: &[u8]) -> crate::Result<IpcResponse> {
  ...
  /// or mutex poisoned).
  pub async fn send_str(&self, op: Op, s: &str) -> crate::Result<IpcResponse> {
    let mut p = Vec::new();
    str_encode(&mut p, s);
  ...
  /// Returns an error if the underlying `send` call fails (timeout, channel dropped,
  /// or mutex poisoned).
  pub async fn send_str_vid(&self, op: Op, s: &str, vid: u64) -> crate::Result<IpcResponse> {
    let mut p = Vec::new();
    str_encode(&mut p, s);
  ...
  /// Returns an error if the underlying `send` call fails (timeout, channel dropped,
  /// or mutex poisoned).
  pub async fn send_vid(&self, op: Op, vid: u64) -> crate::Result<IpcResponse> {
