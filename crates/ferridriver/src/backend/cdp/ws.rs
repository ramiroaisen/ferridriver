  ...
  pub async fn connect_with_headers(
    ...
    for (key, value) in headers {
      let header_name = http::header::HeaderName::from_bytes(key.as_bytes())
        .map_err(|e| FerriError::invalid_argument("header name", format!("'{key}': {e}")))?;
      let header_value = http::header::HeaderValue::from_str(value)
        .map_err(|e| FerriError::invalid_argument("header value", format!("'{key}': {e}")))?;
...
impl super::transport::CdpTransport for WsTransport {
  ...
  async fn send_command(
    ...
    let text = String::from_utf8(data).map_err(|e| FerriError::backend(format!("CDP WebSocket UTF-8: {e}")))?;
    self
      .write_tx
      .send(Message::Text(text.into()))
      .await
      .map_err(|_| FerriError::backend("WebSocket CDP writer closed"))?;
...
async fn discover_ws_url(port_file: &Path, child: &mut tokio::process::Child) -> crate::Result<String> {
  let deadline = tokio::time::Instant::now() + std::time::Duration::from_secs(10);
  loop {
    if tokio::time::Instant::now() >= deadline {
      return Err(FerriError::timeout_plain(10_000));
    }
